...
assert.ok(js.includes("gapMs <= maxGapMs") && js.includes("previousGroup.identityKey === identityKey"), "逐字稿合并必须同时校验相邻身份和按业务类型传入的间隔阈值");
assert.ok(js.includes("maxGapMs: imported ? 6000 : 3000"), "导入转写应容纳离线分离产生的短静音，实时会议仍须保持3秒边界");
...
assert.ok(js.includes("hydrateSelectedImportFileMetadata") && js.includes("readLocalMediaDurationMs"), "导入台账必须读取真实媒体时长，不能写死待识别");
assert.ok(!js.includes('createdAt: "刚刚"') && !js.includes('duration: "待识别"'), "导入时间列不能继续使用写死占位值");
